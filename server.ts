...
interface KeyState {
  ...
  key: string;
  maskedKey: string;
  status: "active" | "rate_limited" | "quota_exceeded" | "failed" | "HARD_LOCKED" | "half_open";
  is_banned?: boolean;
  errorCount: number;
  usageCount: number;
  lastUsed: Date | null;
...
async function refreshApiToggles() {
  const now = Date.now();
  if (now - lastConfigFetchTime < 300000) return; // cache 300 seconds
  ...
  if (!selectedState) {
    ...
    if (limitedKeys.length > 0) {
      ...
      if (nonBannedKeys.length === 0) {
        geminiKeyStates.forEach(s => { s.status = "active"; s.is_banned = false; });
...
function getOpenRouterKey(): { key: string; state: KeyState } {
  ...
  if (!selectedState) {
    let nonBannedKeys = openRouterKeyStates.filter(s => s.status !== "HARD_LOCKED" && !s.is_banned);
    if (nonBannedKeys.length === 0) {
      openRouterKeyStates.forEach(s => {
        s.status = "active";
        s.is_banned = false;
      });
      nonBannedKeys = openRouterKeyStates;
    }
    selectedState = nonBannedKeys[0];
    addOpenRouterRotationLog({
  ...
  updateKeyMetrics(selectedState.index + 100, "usage"); // +100 offset for OpenRouter
...
function getGroqKey(): { key: string; state: KeyState } {
  if (groqKeyStates.length === 0) {
    ...
    if (groqKeyStates.length === 0) {
      const err: any = new Error("LỖI API KEY: Hệ thống đang chạy ở chế độ BYOK (Bring Your Own Key). Vui lòng nhập Cerebras API Key trực tiếp trên giao diện web."); err.status = 401; throw err;
    }
  }
  ...
  while (attempts < groqKeyStates.length) {
    ...
    if (s.status === "active" || s.status === "half_open") {
        selectedState = s;
        if (s.status === "half_open") {
            s.status = "rate_limited"; // Lock key while testing
            s.lastUsed = new Date();
        }
        break;
    }
  ...
  if (!selectedState) {
    throw new Error("503 - Dịch vụ AI đang tạm quá tải/bảo trì. Vui lòng quay lại sau 2 phút.");
  } else if (skippedIndices.length > 0) {
...
let cachedCerebrasModel = "llama3.1-8b";
...
async function executeGenerateContentRoundRobin(contents: any, config: any = {}): Promise<string> {
  ...
  const numKeys = Math.max(1, Math.min(groqKeyStates.length, 2));
  let attempts = config.byokKey ? 1 : numKeys;
  for (let kIdx = 0; kIdx < attempts; kIdx++) {
    let currentGroqState: KeyState | null = null;
    try {
      let key = config.byokKey;
      if (!key) {
        const groqData = getGroqKey();
        key = groqData.key;
        currentGroqState = groqData.state;
      }
      ...
      if (!res.ok) {
        ...
        const error: any = new Error(`Cerebras API Error ${res.status}: ${errBody}`);
      ...
      if (text) {
        if (currentGroqState) { currentGroqState.status = "active"; }
        if (config.byokKey && config.userId && admin.apps.length > 0) {
           try {
              const uId = config.userId;
              if (uId && uId !== "anonymous") {
                 const dateStr = new Date().toISOString().split('T')[0]; 
                 admin.firestore().collection('users').doc(uId).collection('ai_usage').doc(dateStr).set({
                     cerebrasCalls: admin.firestore.FieldValue.increment(1)
                 }, { merge: true }).catch(() => {});
              }
           } catch (e) {}
        }
      ...
      if (currentGroqState) {
        if (isAbort) {
          // Timeout is not a hard provider failure, but we log it.
          // If it was half_open (temporarily rate_limited), we leave it rate_limited (failed).
          // If it was active, we leave it active so it doesn't exhaust the pool.
          
          if (currentGroqState.status === "rate_limited" && err.name === "AbortError") {
             // Let it stay in cooldown if it timed out during half-open probe
          }
        } else {
          handleGroqError(currentGroqState, err);
        }
      }
      ...
      // FAIL FAST
      if (errStatus === 400 || errStatus === 401 || errStatus === 403 || errStatus === 404 || errStatus === 422 || msg.includes("Content Safety") || msg.includes("503 - Dịch vụ AI")) {
        throw finalError; // Break loop immediately
      }
    }
  }
...
// JWT Helper for Firebase ID Tokens
  ...
  const getUserProfileFromFirestore = async (userId: string, idToken: string): Promise<{role: string | null, isPro: boolean, lastAiUsedDate?: string, aiLimitUsedToday?: number, unitedEngineUses?: number}> => {
    if (admin.apps.length > 0) {
      try {
        const db = admin.firestore();
        ...
        if (docSnap.exists) {
           ...
           return {
             role: data?.role || null,
             isPro: !!data?.isPro,
             lastAiUsedDate: data?.lastAiUsedDate,
             aiLimitUsedToday: data?.aiLimitUsedToday,
             unitedEngineUses: data?.unitedEngineUses
           };
        } else {
    ...
    try {
      ...
      const lastAiUsedDate = docData?.fields?.lastAiUsedDate?.stringValue;
      const aiLimitUsedToday = docData?.fields?.aiLimitUsedToday?.integerValue ? parseInt(docData.fields.aiLimitUsedToday.integerValue, 10) : undefined;
      const unitedEngineUses = docData?.fields?.unitedEngineUses?.integerValue ? parseInt(docData.fields.unitedEngineUses.integerValue, 10) : (docData?.fields?.unitedEngineUses?.doubleValue ? parseInt(docData.fields.unitedEngineUses.doubleValue, 10) : undefined);
      return { role, isPro, lastAiUsedDate, aiLimitUsedToday, unitedEngineUses };
    } catch (error) {
  ...
  const isAiLockedForRequest = (reqPath: string, userId: string): { busy: boolean; message?: string } => {
    ...
    if (activeAiTaskType && now < activeAiTaskExpiry) {
      ...
      return {
        busy: true,
        message: `Hệ thống AI đang bận xử lý tiến trình "${taskName}". Để tránh quá tải API và bảo vệ tính ổn định, mọi chức năng AI khác tạm thời bị khóa.`
      };
    }
    return { busy: false };
  };

  const checkAndUpdateAiQuota = async (userId: string, isPro: boolean, userRole: string, profileData?: any): Promise<{ allowed: boolean; message?: string; used?: number }> => {
    ...
    if (admin.apps.length > 0) {
      try {
        ...
        const AI_QUOTA_LIMIT = 5; // Default free user limit
        let aiLimitUsedToday = 0;
        let lastAiUsedDate = todayStr;
        let unitedEngineUses = 0;

        if (profileData) {
          lastAiUsedDate = profileData.lastAiUsedDate || todayStr;
          aiLimitUsedToday = profileData.aiLimitUsedToday || 0;
          unitedEngineUses = typeof profileData.unitedEngineUses === 'number' ? profileData.unitedEngineUses : 0;
  ...
  const aiCooldownMiddleware = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    if (userId && userRole === "student" && !isPro) {
      const quotaCheck = await checkAndUpdateAiQuota(userId, isPro, userRole, profileData);
...
text;
      ...
      let responseText = await executeGenerateContentRoundRobin(prompt, { byokKey: req?.headers?.["x-cerebras-key"], userId: req?.headers?.["x-user-id"] || (req as any).userId || (req as any).user?.uid });
...
Bọc công thức Toán/Lý/Hóa bằng LaTeX (dấu $ hoặc $$). Chỉ trả ra nội dung (markdown).`;
      ...
      let responseText = "";
      try {
        responseText = await executeGenerateContentRoundRobin(prompt, { byokKey: req?.headers?.["x-cerebras-key"], userId: req?.headers?.["x-user-id"] || (req as any).userId || (req as any).user?.uid });
...
[
  {
    ...
    "explanation": "string - Giải thích ngắn vì sao lại chọn đáp án này"
      ...
      const responseText = await executeGenerateContentRoundRobin(prompt, { byokKey: req?.headers?.["x-cerebras-key"], userId: req?.headers?.["x-user-id"] || (req as any).userId || (req as any).user?.uid, 
...
${chunkWords.join("\n")}`;
         ...
         while (retryAttempts < 3 && !parseSuccess) {
            try {
               const chunkResText = await executeGenerateContentRoundRobin(prompt, { byokKey: req?.headers?.["x-cerebras-key"], userId: req?.headers?.["x-user-id"] || (req as any).userId || (req as any).user?.uid, temperature: 0.1 });
...
BẮT BUỘC TRẢ VỀ ĐÚNG MỘT OBJECT JSON DUY NHẤT (không bọc markdown code block, không câu dẫn):
{
  ...
  "formattedExample": "Ví dụ đã được định dạng (nếu có)"
      ...
      console.warn("Format Card Error:", error.message);
      next(error);
    }
  });
...
${conciseModeGuidance}`;
      ...
      if (mode === "quiz") {
          ...
          if (mcqData) {
            ...
            let responseText = "";
            try {
              responseText = await executeGenerateContentRoundRobin(mcqPrompt, { byokKey: req?.headers?.["x-cerebras-key"], userId: req?.headers?.["x-user-id"] || (req as any).userId || (req as any).user?.uid, 
                 responseMimeType: "application/json"
              });
            } catch (geminiError: any) {
              throw geminiError;
...
${reminderSuffix}`;
      ...
      let responseText = "";
      try {
        responseText = await executeGenerateContentRoundRobin(contents, { byokKey: req?.headers?.["x-cerebras-key"], userId: req?.headers?.["x-user-id"] || (req as any).userId || (req as any).user?.uid, 
      ...
      console.warn("Agent 3 Error:", error.message);
      next(error);
    }
  });
  ...
  app.post("/api/automation/reset-weekly-points", async (req, res, next) => {
    try {
      ...
      const lockRef = db.collection("system_config").doc("weekly_reset_state");
      
      console.log(`🤖 [Auto Weekly Reset] Khởi chạy dọn dẹp điểm tuần toàn hệ thống cho tuần: ${currentWeekId} (Exec: ${executionId})`);
      
      // 1. Acquire Lock with Fencing (Transaction)
      const lockResult = await db.runTransaction(async (t) => {
        const doc = await t.get(lockRef);
        const now = Date.now();
        if (doc.exists) {
          const data = doc.data();
          if (data?.lastResetWeek === currentWeekId) {
            if (data?.status === "completed") {
              return { acquired: false, reason: "already_done" };
            }
            if (data?.status === "processing") {
              const lockedAt = data.lockedAt || 0;
              // Takeover if > 5 minutes (300,000 ms)
              if (now - lockedAt < 300000) {
                return { acquired: false, reason: "in_progress" };
              }
              // Takeover
              t.set(lockRef, { lastResetWeek: currentWeekId, status: "processing", lockedAt: now, ownerId: executionId }, { merge: true });
              return { acquired: true, reason: "takeover" };
            }
          }
        }
        // New lock
      ...
      if (!lockResult.acquired) {
        console.log(`🤖 [Auto Weekly Reset] Bỏ qua: ${lockResult.reason}`);
        return res.json({ success: true, message: `Bỏ qua: ${lockResult.reason}`, updatedCount: 0, weekId: currentWeekId });
      }

      // 2. Query Users (M-Read) - Only Owner runs this
      const usersSnapshot = await db.collection("users").where("points", ">", 0).get();
      let updatedCount = 0;

      if (!usersSnapshot.empty) {
        // Filter docs that actually need update
        const docsToUpdate = usersSnapshot.docs.filter(doc => {
          const uData = doc.data();
          return uData.lastWeeklyResetWeek !== currentWeekId;
        });
        ...
        for (let i = 0; i < docsToUpdate.length; i += CHUNK_SIZE) {
          ...
          chunk.forEach(doc => {
            batch.update(doc.ref, { points: 0, lastWeeklyResetWeek: currentWeekId });
            updatedCount++;
          });
          await batch.commit();
          console.log(`🤖 [Auto Weekly Reset] Đã commit chunk ${Math.floor(i / CHUNK_SIZE) + 1} (${chunk.length} users)`);
        }
      }

      // 3. Complete with Fencing
      await db.runTransaction(async (t) => {
        const doc = await t.get(lockRef);
        if (doc.exists && doc.data()?.ownerId === executionId) {
          t.set(lockRef, { status: "completed" }, { merge: true });
        } else {
          throw new Error("FENCED: Owner ID mismatch or doc missing. Execution was taken over.");
        }
      });
      ...
      if (err.message && err.message.includes("FENCED")) {
        console.error(`🤖 [Auto Weekly Reset] Bị chặn bởi Fencing: ${err.message}`);
...
${textChunk}`;
        ...
        const responseTextObj = await executeGenerateContentRoundRobin(activePrompt, { byokKey: req?.headers?.["x-cerebras-key"], userId: req?.headers?.["x-user-id"] || (req as any).userId || (req as any).user?.uid, 
...
Do not include any markdown wrapper or extra text.`;
      ...
      const responseText = await executeGenerateContentRoundRobin(requestPrompt, { byokKey: req?.headers?.["x-cerebras-key"], userId: req?.headers?.["x-user-id"] || (req as any).userId || (req as any).user?.uid, 
         responseMimeType: "application/json",
         temperature: 0.1
      });
...
Trả về dữ liệu dưới dạng JSON chuẩn (không dùng markdown block) với cấu trúc:
{
  ...
  "wordForm": "Loại từ, phát âm IPA (chỉ áp dụng nếu là tiếng Anh)"
      ...
      console.warn("Manual Define Error:", err.message);
...
${jsonText}`;
      ...
      console.warn("Automation validate-json error:", error.message);
      next(error);
    }
  });
...
app.use((err: any, req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  if (statusCode === 401) { console.warn("Expected API Key Warning:", err.message); } else { console.error("Global Error Caught:", err); }
...
app.get("/api/usage/cerebras", async (req, res) => {
    try {
        const userId = req.headers["x-user-id"] || (req as any).userId || (req as any).user?.uid;
        if (!userId || userId === "anonymous" || admin.apps.length === 0) {
        ...
        const snapshot = await admin.firestore().collection('users').doc(userId).collection('ai_usage')
            .where(admin.firestore.FieldPath.documentId(), '>=', limitDateStr)
            .get();
            
        const data = snapshot.docs.map(doc => ({ date: doc.id, calls: doc.data().cerebrasCalls || 0 }));
        return res.json({ success: true, data });
    } catch (e) {
        return res.json({ success: true, data: [] });
    }
